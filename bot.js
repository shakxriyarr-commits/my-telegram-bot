...
bot.hears('🏠 Mijoz menyusiga o\'tish', (ctx) => ctx.reply("Mijoz rejimi menyusi:", mainKeyboard));

// --- ADMIN HISOBOT VA BUYURTMALAR ---
...
bot.hears('📦 Faol buyurtmalar', (ctx) => {
    ...
    ctx.reply(`Hozirda ${activeOrders.length} ta kuryerga topshirilmagan buyurtma bor:`);
...
// --- BEKOR QILISH VA LOCK ---
bot.action(/lock_(.+)/, async (ctx) => {
    const orderId = ctx.match[1];
    const order = orders[orderId];
    if (order) {
        order.lockCancel = true;
        await ctx.telegram.sendMessage(order.userId, `👨‍🍳 #${orderId} buyurtmangiz tayyorlanmoqda. Endi bekor qilib bo'lmaydi! 🔒`);
        ctx.editMessageReplyMarkup(Markup.inlineKeyboard([
            [Markup.button.callback("🚗 Kuryerga", `sd_${orderId}`)],
            [Markup.button.callback("⚠️ Mahsulot tugagan", `ed_${orderId}`)]
        ]).reply_markup);
        ctx.answerCbQuery("Mijozga blok xabari yuborildi!");
    }
});
...
bot.action(/cn_(.+)/, async (ctx) => {
    ...
    if (order) {
        ...
        ctx.editMessageText(`🚫 Buyurtma #${orderId} bekor qilindi va mijozga xabar berildi.`);
    }
});
...
bot.hears('🗂 Buyurtmalarim', (ctx) => {
    ...
    if (userOrders.length === 0) return ctx.reply("Faol buyurtmalaringiz yo'q.");
...
bot.action(/u_cn_(.+)/, async (ctx) => {
    ...
    } else ctx.answerCbQuery("Bekor qilish allaqachon bloklangan! 🔒");
});

// --- KURYER VA TUGAGAN MAHSULOT ---
bot.action(/sd_(.+)/, (ctx) => {
    const orderId = ctx.match[1];
...
bot.action(/ch_(.+)_(.+)/, async (ctx) => {
    ...
    await ctx.telegram.sendMessage(courierId, `📦 YANGI BUYURTMA #${orderId}\n📞 +${order.phone}\n💰 ${order.total}`, {
        ...Markup.inlineKeyboard([[Markup.button.callback("✅ Qabul qildim", `c_take_${orderId}`)], [Markup.button.callback("🏁 Topshirdim", `c_done_${orderId}`)]])
    });
    await ctx.telegram.sendLocation(courierId, order.latitude, order.longitude);
    ctx.editMessageText(`✅ #${orderId} kuryerga berildi.`);
});
...
bot.action(/c_done_(.+)/, async (ctx) => {
    ...
    if (order) {
        ...
        await ctx.telegram.sendMessage(ADMIN_ID, `✅ #${orderId} topshirildi va statistikaga qo'shildi.`);
