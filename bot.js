...
bot.action('confirm_card_pay', async (ctx) => {
    const userId = ctx.from.id;
    // FOYDALANUVCHI MA'LUMOTINI VA PENDING ORDERNI TEKSHIRISH
    const pending = users[userId] ? users[userId].pendingOrder : null;

    if (!pending) {
        return ctx.answerCbQuery("⚠️ Savatcha topilmadi! Iltimos qaytadan menyuga kiring.", { show_alert: true });
    }
...
// --- ADMIN VA KURYER AMALLARI (HAMMASI JOYIDA) ---
...
bot.action(/lock_(.+)/, async (ctx) => {
    ...
    if (orders[id]) {
        ...
        bot.telegram.sendMessage(orders[id].userId, `👨‍🍳 Buyurtma #${id} tayyorlanishni boshladi! 🔒`);
...
bot.action(/c_out_(.+)_(.+)/, async (ctx) => {
    ...
    if (order) {
        ...
        bot.telegram.sendMessage(order.userId, `⚠️ Uzr, *${item.name}* tugagan ekan. Buyurtma qayta hisoblandi.`);
...
bot.hears('🏁 Topshirilgan buyurtmalarim', (ctx) => ctx.reply(`✅ Jami topshirilgan: ${courierStats[ctx.from.id] || 0} ta`));
...
bot.telegram.deleteWebhook().then(() => {
    bot.launch();
    console.log("Bot 100% ideal holatda ishga tushdi!");
});
