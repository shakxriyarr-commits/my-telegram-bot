...
bot.on('text', (ctx, next) => {
    ...
    if (state.step === 'name') {
        ...
        state.step = 'price';
        ctx.reply(`${state.name} uchun narxni kiriting (faqat raqam):`);
        ...
        ctx.reply("✅ Yangi taom menyuga muvaffaqiyatli qo'shildi!", adminKeyboard);
    }
});
...
bot.action('pay_card', async (ctx) => {
    ...
    const total = cart.reduce((a, b) => a + b.price, 0);
    // Ma'lumotlarni yo'qotmaslik uchun users ichiga backup qilamiz
...
bot.action('confirm_card', async (ctx) => {
    ...
    if (!pending) {
        return ctx.answerCbQuery("⚠️ Xatolik! Savatchangiz ma'lumotlari topilmadi. Iltimos, menyuga qaytib, qaytadan urinib ko'ring.", { show_alert: true });
    }
...
// --- ADMIN ACTIONLARI (TO'LIQ MATNLAR BILAN) ---
...
bot.action(/out_list_(.+)/, (ctx) => {
    ...
    if (orders[id]) {
        ...
        ctx.editMessageText("Tugab qolgan mahsulotni tanlang:", Markup.inlineKeyboard(btns));
    }
});
...
bot.action(/c_take_(.+)/, (ctx) => {
    const id = ctx.match[1];
    if (orders[id]) {
        bot.telegram.sendMessage(orders[id].userId, `🚀 Buyurtmangiz yo'lda! Kuryer uni siz tomon olib kelmoqda. 🚚`);
...
bot.action(/c_done_(.+)/, (ctx) => {
    ...
    if (o) {
        ...
        bot.telegram.sendMessage(o.userId, `🏁 Buyurtmangiz muvaffaqiyatli yetkazildi! Bizni tanlaganingiz uchun rahmat. Yoqimli ishtaha! 👋`);
        bot.telegram.sendMessage(ADMIN_ID, `✅ Buyurtma #${id} muvaffaqiyatli topshirildi.`);
...
bot.hears('🏁 Topshirilgan buyurtmalarim', (ctx) => {
    ctx.reply(`✅ Siz bugun jami ${courierStats[ctx.from.id] || 0} ta buyurtma yetkazib berdingiz. Charchamang!`);
});

bot.hears('🏠 Mijoz menyusiga o\'tish', (ctx) => ctx.reply("Mijoz menyusiga o'tildi:", mainKeyboard));
bot.action('clear_cart', (ctx) => { carts[ctx.from.id] = []; ctx.editMessageText("🛒 Savatchangiz muvaffaqiyatli tozalandi."); });

// --- LAUNCH ---
bot.telegram.deleteWebhook().then(() => {
    bot.launch();
    console.log("BOT 100% TO'LIQ VA IDEAL HOLATDA ISHGA TUSHDI! 🚀");
});
